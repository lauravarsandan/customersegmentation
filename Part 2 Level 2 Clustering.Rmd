...
Let's first see how the words look with 11 clusters then 4

```{r}
cl3_clara_words <- clara(topic_matrix,4,metric="euclidean",samples=100)
...
```{r}
cl5_clara_words <- clara(topic_matrix,5,metric="euclidean",samples=100)
...
p <- ggplot(cl_all, aes(word, tf, fill = clusters)) +
  geom_col(show.legend = FALSE) +
  labs(x = NULL, y = "tf") +
  facet_wrap(~clusters, ncol = 2, scales = "free") +
  coord_flip()
...
book_words %>%
  ...
  top_n(10) %>% 
  ungroup %>%
  ggplot(aes(word, tf, fill = clusters)) +
  geom_col(show.legend = FALSE) +
  labs(x = NULL, y = "tf") +
  facet_wrap(~clusters, ncol = 2, scales = "free") +
  coord_flip()
